...
export default function AdminDash(){
  ...
  const load=async()=>{
    const p=await api('/api/tenant/profile'); setProfile(p.profile||{})
    const pr=await api('/api/ota/providers'); setProv(pr.items||[])
    const cr=await api('/api/ota/credentials'); setCreds(cr.items||[])
  }
  useEffect(()=>{ load() },[])

  const saveProfile=async(e)=>{
    e.preventDefault()
    await api('/api/tenant/profile',{ method:'POST', body: JSON.stringify(profile) })
    alert('Saved profile')
  }

  const addCred=async(e)=>{
    e.preventDefault()
    const providerId=e.target.providerId.value; const propertyCode=e.target.propertyCode.value; const username=e.target.username.value; const secret=e.target.secret.value
    await api('/api/ota/credentials',{ method:'POST', body: JSON.stringify({ providerId, propertyCode, username, secret }) })
    e.target.reset(); load()
  }
  ...
  return <div style={{maxWidth:960, margin:'2rem auto'}}>
    ...
    <section style={{background:'#fff', padding:16, borderRadius:12}}>
      ...
      <ul>
        {creds.map(c=> <li key={c.id}>{c.providerName} — {c.propertyCode} ({c.enabled?'enabled':'disabled'})</li>)}
      </ul>
    </section>
  </div>
}
