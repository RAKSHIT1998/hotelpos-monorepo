...
export default function RatePlans(){
  ...
  const [code,setCode]=useState('STD'); const [name,setName]=useState('Standard'); const [currency,setCurrency]=useState('INR'); const [baseMinor,setBase]=useState(500000)
  ...
  return <div style={{maxWidth:800, margin:'2rem auto'}}>
    ...
    <ul>
      {items.map(x=> <li key={x.id}>{x.code} — {x.name} — {x.currency} — {(x.baseMinor/100).toFixed(2)}</li>)}
    </ul>
  </div>
}
